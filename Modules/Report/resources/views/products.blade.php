...
<div class="report-shell">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-4 border-bottom border-slate-200 pb-5 mb-8 report-header">
        <div>
            <span class="report-kicker"><i class="bi bi-box-seam"></i> {{ __('app.analytics') }}</span>
            <h1 class="dashboard-title display-font text-3xl font-black text-slate-900 leading-none mb-1">{{ __('report.products_report') }}</h1>
            <p class="dashboard-subtitle text-slate-500 text-sm mt-1.5">{{ __('report.products_report_subtitle') }}</p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('report.export.products-pdf', request()->query()) }}" class="btn-brand-custom" style="background: linear-gradient(140deg, #ef4444, #b91c1c); box-shadow: 0 14px 28px rgba(239, 68, 68, 0.28);">
    ...
    <div class="row mb-4 g-3">
        <div class="col-lg-2 col-md-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.total_products') }}</div>
                <div class="stat-value">{{ $stockSummary['total_products'] }}</div>
            </div>
        </div>
        <div class="col-lg-2 col-md-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.stock_value') }}</div>
                <div class="stat-value text-teal" style="color: #0f766e !important;">{{ number_format($stockSummary['total_stock_value'], 2) }}</div>
            </div>
        </div>
        <div class="col-lg-2 col-md-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.potential_revenue') }}</div>
                <div class="stat-value">{{ number_format($stockSummary['total_potential_revenue'], 2) }}</div>
            </div>
        </div>
        <div class="col-lg-2 col-md-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.potential_profit') }}</div>
                <div class="stat-value text-success">{{ number_format($stockSummary['total_potential_profit'], 2) }}</div>
            </div>
        </div>
        <div class="col-lg-2 col-md-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.low_stock_label') }}</div>
                <div class="stat-value text-warning">{{ $stockSummary['low_stock_count'] }}</div>
            </div>
        </div>
        <div class="col-lg-2 col-md-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.out_of_stock') }}</div>
                <div class="stat-value text-danger">{{ $stockSummary['out_of_stock_count'] }}</div>
            </div>
        </div>
    </div>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge bg-light text-dark border">{{ $product->shop->name }}</span></td>
                        <td class="text-end fw-semibold">{{ number_format($product->purchase_price, 2) }}</td>
                        <td class="text-end fw-semibold">{{ number_format($product->sale_price, 2) }}</td>
                        <td class="text-center fw-bold">
                            @if($product->stock_quantity == 0)
                                <span class="badge bg-danger-subtle text-danger border border-danger-subtle rounded-pill px-2.5">{{ __('report.out_of_stock_badge') }}</span>
                            @elseif($product->stock_quantity <= (int) \Modules\Settings\Models\Setting::get('low_stock_alert', 5))
                                <span class="badge bg-warning-subtle text-warning border border-warning-subtle rounded-pill px-2.5">{{ $product->stock_quantity }}</span>
                            @else
                                <span class="badge bg-success-subtle text-success border border-success-subtle rounded-pill px-2.5">{{ $product->stock_quantity }}</span>
                            @endif
                        </td>
                        <td class="text-end font-monospace fw-bold">{{ number_format($product->stock_quantity * $product->purchase_price, 2) }}</td>
                        <td class="text-center font-monospace">{{ $product->total_units_sold }}</td>
                        <td class="text-end text-success font-monospace fw-bold">{{ number_format($product->total_revenue, 2) }}</td>
                        <td class="text-center">
                            ...
                            <button
                                ...
                                data-product='@json($productData, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_TAG)'
                            >
                                <i class="bi bi-eye"></i>
                            </button>
                        </td>
