...
@section('title', __('report.exchanges_report'))
...
<div class="report-shell">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-4 border-bottom border-slate-200 pb-5 mb-8 report-header">
        <div>
            <span class="report-kicker"><i class="bi bi-arrow-left-right"></i> {{ __('app.analytics') }}</span>
            <h1 class="dashboard-title display-font text-3xl font-black text-slate-900 leading-none mb-1">{{ __('report.exchanges_report') }}</h1>
            <p class="dashboard-subtitle text-slate-500 text-sm mt-1.5">{{ __('report.exchanges_report_subtitle') }}</p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('report.export.exchanges-pdf', request()->query()) }}" class="btn-brand-custom" style="background: linear-gradient(140deg, #ef4444, #b91c1c); box-shadow: 0 14px 28px rgba(239, 68, 68, 0.28);">
    ...
    <div class="content-card mb-4">
        ...
        <div class="p-4">
            <form action="{{ route('report.exchanges') }}" method="GET">
                <div class="row g-3">
                    <div class="col-md-3">
                        <label for="shop_id" class="form-label fw-semibold text-secondary small text-uppercase tracking-wider">{{ __('report.shop') }}</label>
                    ...
                    <div class="col-md-3">
                        <label for="exchange_type" class="form-label fw-semibold text-secondary small text-uppercase tracking-wider">{{ __('sale.exchange_type') }}</label>
                        <select class="form-select" id="exchange_type" name="exchange_type" style="border-radius: 10px; border-color: #cedce9; background-color: #f8fafc;">
                            ...
                            <option value="replacement" {{ ($filters['exchange_type'] ?? '') === 'replacement' ? 'selected' : '' }}>
                                {{ __('sale.exchange_type_replacement') }}
                            </option>
                            <option value="return_only" {{ ($filters['exchange_type'] ?? '') === 'return_only' ? 'selected' : '' }}>
                                {{ __('sale.exchange_type_return_only') }}
                            </option>
                        </select>
                    </div>
                    <div class="col-md-2">
    ...
    <div class="row mb-4 g-3">
        <div class="col-md-3 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.exchanges_count') }}</div>
                <div class="stat-value">{{ $exchangeSummary->total ?? 0 }}</div>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.replacements_count') }}</div>
                <div class="stat-value text-teal" style="color: #0f766e !important;">{{ $exchangeSummary->replacements ?? 0 }}</div>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.returns_count') }}</div>
                <div class="stat-value text-warning">{{ $exchangeSummary->returns ?? 0 }}</div>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.cost_difference') }}</div>
    ...
    <div class="content-card">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    @endphp
                    <tr>
                        <td><strong class="text-teal"><i class="bi bi-receipt"></i> #{{ $exchange->sale_id }}</strong></td>
                        <td><span class="badge bg-light text-dark border">{{ $exchange->shop->name }}</span></td>
                        <td>
                            <strong>{{ $exchange->originalBatch->product->name ?? ($exchange->sale->product->name ?? '-') }}</strong>
                            @if($exchange->originalBatch && $exchange->originalBatch->attribute_summary !== '-')
                                <div class="text-muted small">{{ $exchange->originalBatch->attribute_summary }}</div>
                            @endif
                        </td>
                        <td><code class="text-dark">{{ $exchange->originalBatch->batch_code ?? '-' }}</code></td>
                        <td>
                            @if($exchange->exchange_type === 'replacement' && $exchange->replacementBatch)
                                <code class="text-dark">{{ $exchange->replacementBatch->batch_code }}</code>
                                @if($exchange->replacementBatch->product && $exchange->replacementBatch->product->id !== ($exchange->originalBatch->product->id ?? null))
                                    <div class="text-muted small">({{ $exchange->replacementBatch->product->name }})</div>
                                @endif
                            @else
                                <span class="text-muted small">-</span>
                            @endif
                        </td>
                        <td class="text-center fw-bold">{{ $exchange->quantity }}</td>
                        <td>
                        ...
                        <td>
                            <span class="exchange-badge exchange-badge-{{ $typeClass }}">
                        ...
                        <td>
                            <span class="text-muted small" title="{{ $exchange->note }}">
                                {{ ucfirst(str_replace('_', ' ', $exchange->reason)) }}
                                @if($exchange->note)
                                    <i class="bi bi-info-circle text-secondary ms-1" data-bs-toggle="tooltip" title="{{ $exchange->note }}"></i>
                                @endif
                            </span>
                        </td>
                        <td class="text-center">
                            <button
                                ...
                                data-exchange='@json($exchangeData, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_TAG)'
                            >
                                <i class="bi bi-eye"></i>
                            </button>
                        </td>
        ...
        <div class="p-5 text-center text-muted">
            ...
            <p class="mb-0">{{ __('report.no_exchanges_found') }}</p>
        </div>
        @endif
    </div>
</div>
