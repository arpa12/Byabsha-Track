...
<div class="report-shell">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-4 border-bottom border-slate-200 pb-5 mb-8 report-header">
        <div>
            <span class="report-kicker"><i class="bi bi-graph-up-arrow"></i> {{ __('app.analytics') }}</span>
            <h1 class="dashboard-title display-font text-3xl font-black text-slate-900 leading-none mb-1">{{ __('report.sales_report') }}</h1>
            <p class="dashboard-subtitle text-slate-500 text-sm mt-1.5">{{ __('report.sales_report_subtitle') }}</p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('report.export.sales-pdf', request()->query()) }}" class="btn-brand-custom" style="background: linear-gradient(140deg, #ef4444, #b91c1c); box-shadow: 0 14px 28px rgba(239, 68, 68, 0.28);">
    ...
    <div class="row mb-4 g-3">
        <div class="col-md-3 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.total_sales') }}</div>
                <div class="stat-value">{{ $salesSummary->total_transactions ?? 0 }}</div>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.total_revenue') }}</div>
                <div class="stat-value text-teal" style="color: #0f766e !important;">{{ currency_symbol() }}{{ number_format($salesSummary->total_revenue ?? 0, 2) }}</div>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.total_profit') }}</div>
                <div class="stat-value text-success">{{ currency_symbol() }}{{ number_format($salesSummary->total_profit ?? 0, 2) }}</div>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.items_sold') }}</div>
                <div class="stat-value text-warning">{{ number_format($salesSummary->total_quantity_sold ?? 0) }}</div>
            </div>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><span class="badge bg-light text-dark border">{{ $sale->shop->name }}</span></td>
                        ...
                        <td class="text-end fw-semibold">{{ currency_symbol() }}{{ number_format($sale->sale_price, 2) }}</td>
                        <td class="text-end fw-bold">{{ currency_symbol() }}{{ number_format($sale->total_amount, 2) }}</td>
                        <td class="text-end text-success fw-bold">{{ currency_symbol() }}{{ number_format($sale->profit, 2) }}</td>
                        <td class="text-center">
                            ...
                            <button
                                ...
                                data-sale='@json($saleData, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_TAG)'
                            >
                                <i class="bi bi-eye"></i>
                            </button>
                        </td>
        ...
        <div class="modal-content report-modal">
            <div class="modal-header">
                <div>
                    ...
                    <h5 class="modal-title mb-0" id="saleDetailsModalTitle">{{ __('report.sale_details') }}</h5>
                </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        <div class="card border-0 bg-light h-100 rounded-3">
                            <div class="card-body">
                                <h6 class="text-uppercase text-secondary small fw-bold mb-3 tracking-wider">{{ __('report.customer_name') }}</h6>
                                <div class="mb-3">
                                    <div class="text-xs text-muted mb-0.5">{{ __('report.customer_name') }}</div>
                                    <div class="fw-bold fs-6" id="modalSaleCustomerName">-</div>
                                </div>
                                <div class="mb-3">
                                    <div class="text-xs text-muted mb-0.5">{{ __('report.customer_phone') }}</div>
                                    <div class="fw-bold" id="modalSaleCustomerPhone">-</div>
                                </div>
                                <div class="mb-0">
                                    <div class="text-xs text-muted mb-0.5">{{ __('report.customer_address') }}</div>
                    ...
                    <div class="col-md-6">
                        <div class="card border-0 bg-light h-100 rounded-3">
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    <div class="text-xs text-muted mb-0.5">{{ __('report.shop') }}</div>
                                    <div class="fw-bold" id="modalSaleShopName">-</div>
                                </div>
                                <div class="mb-3">
                                    <div class="text-xs text-muted mb-0.5">{{ __('report.product_name') }}</div>
