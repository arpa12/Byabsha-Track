...
<div class="sale-shell">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3 sale-header">
        <div>
            ...
            <h1 class="page-title display-font mb-1"><i class="bi bi-arrow-left-right"></i> <?php echo e(__('sale.exchange_title')); ?></h1>
            <p class="page-subtitle mb-0"><?php echo e(__('sale.exchange_subtitle')); ?></p>
        </div>
        <a href="<?php echo e(route('sale.exchanges.create')); ?>" class="btn-new-sale shadow-sm">
    ...
    <div class="filter-toolbar mb-4 shadow-sm">
        <form method="GET" action="<?php echo e(route('sale.exchanges.index')); ?>" class="m-0 w-100" id="filterForm">
            <div class="row g-2 align-items-center">
                ...
                <div class="col-md-3">
                    <select name="type" id="type" class="form-select filter-select-control">
    ...
    <div class="product-table-wrap p-3">
        <div class="table-responsive">
            <table class="table align-middle mb-0 w-100" id="exchangesDataTable">
                <thead>
                    <tr>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        if (!window.jQuery || !$('#exchangesDataTable').length) {
            return;
        }

        var table = $('#exchangesDataTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '<?php echo e(route("sale.exchanges.table")); ?>',
                data: function (d) {
            ...
            columns: [
                { data: 'sale_reference_label', name: 'sale_exchanges.sale_id' },
                { data: 'shop_name_label', name: 'shops.name' },
                { data: 'product_name', name: 'products.name' },
                { data: 'exchange_date_label', name: 'sale_exchanges.exchange_date' },
                { data: 'exchange_type_label', name: 'sale_exchanges.exchange_type' },
                { data: 'quantity', name: 'sale_exchanges.quantity', className: 'text-center fw-bold' },
                { data: 'cost_difference_label', name: 'sale_exchanges.cost_difference', className: 'text-end' },
                { data: 'reason', name: 'sale_exchanges.reason' }
            ],
            pageLength: 20,
            order: [[3, 'desc']],
            responsive: true,
        ...
        $('#shop_id, #type').on('change', function () {
            table.ajax.reload();
        });
    });
</script>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\personal\Byabsha-Track\Modules/Sale\resources/views/exchanges/index.blade.php ENDPATH**/ ?>