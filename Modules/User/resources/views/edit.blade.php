...
<div class="mb-4">
    <span class="form-kicker"><i class="bi bi-pencil-square"></i>{{ __('user.edit_title') }}</span>
    <h1 class="page-title display-font">{{ __('user.edit_title') }}</h1>
    <p class="page-subtitle">{{ __('user.edit_subtitle') }}</p>
</div>
    ...
    <form action="{{ route('user.update', $user->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            @error('password')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <small class="helper-text">{{ __('user.password_hint_edit') }}</small>
        </div>
        ...
        <div class="d-flex justify-content-between">
            ...
            <button type="submit" class="btn btn-submit"><i class="bi bi-check-circle"></i>{{ __('app.update') }}</button>
        </div>
    </form>
</div>
</div>
