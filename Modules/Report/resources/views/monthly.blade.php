...
<div class="monthly-shell">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-4 border-bottom border-slate-200 pb-5 mb-8 report-header">
        <div>
            <span class="report-kicker"><i class="bi bi-calendar-range"></i> {{ __('app.analytics') }}</span>
            @php
                $currentShop = $shops->firstWhere('id', $filters['shop_id']);
                $shopDisplay = $currentShop ? $currentShop->name : __('report.all_shops');
            @endphp
            <h1 class="dashboard-title display-font text-3xl font-black text-slate-900 leading-none mb-1">{{ __('report.monthly_pnl') }}</h1>
            <p class="dashboard-subtitle text-slate-500 text-sm mt-1.5">{{ __('report.monthly_pnl_subtitle') }} • <strong>{{ __('report.shop') }}:</strong> {{ $shopDisplay }}</p>
        </div>
