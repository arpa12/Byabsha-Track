...
@section('title', __('report.warranties_report'))
...
<div class="report-shell">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-4 border-bottom border-slate-200 pb-5 mb-8 report-header">
        <div>
            <span class="report-kicker"><i class="bi bi-shield-check"></i> {{ __('app.analytics') }}</span>
            <h1 class="dashboard-title display-font text-3xl font-black text-slate-900 leading-none mb-1">{{ __('report.warranties_report') }}</h1>
            <p class="dashboard-subtitle text-slate-500 text-sm mt-1.5">{{ __('report.warranties_report_subtitle') }}</p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('report.export.warranties-pdf', request()->query()) }}" class="btn-brand-custom" style="background: linear-gradient(140deg, #ef4444, #b91c1c); box-shadow: 0 14px 28px rgba(239, 68, 68, 0.28);">
    ...
    <div class="content-card mb-4">
        ...
        <div class="p-4">
            <form action="{{ route('report.warranties') }}" method="GET">
                <div class="row g-3">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        <label for="status" class="form-label fw-semibold text-secondary small text-uppercase tracking-wider">{{ __('sale.warranty_status') }}</label>
                        <select class="form-select" id="status" name="status" style="border-radius: 10px; border-color: #cedce9; background-color: #f8fafc;">
                            ...
                            @foreach(['active', 'claimed', 'expired', 'void'] as $statusOption)
                                <option value="{{ $statusOption }}" {{ ($filters['status'] ?? '') === $statusOption ? 'selected' : '' }}>
                                    {{ __('sale.status_' . $statusOption) }}
                                </option>
                            @endforeach
                        </select>
                    </div>
    ...
    <div class="row mb-4 g-3">
        <div class="col-md-2-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('report.total_warranties') }}</div>
                <div class="stat-value">{{ $warrantySummary->total ?? 0 }}</div>
            </div>
        </div>
        <div class="col-md-2-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('sale.status_active') }}</div>
                <div class="stat-value text-success">{{ $warrantySummary->active ?? 0 }}</div>
            </div>
        </div>
        <div class="col-md-2-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('sale.status_expired') }}</div>
                <div class="stat-value text-warning">{{ $warrantySummary->expired ?? 0 }}</div>
            </div>
        </div>
        <div class="col-md-2-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('sale.status_claimed') }}</div>
                <div class="stat-value text-info">{{ $warrantySummary->claimed ?? 0 }}</div>
            </div>
        </div>
        <div class="col-md-2-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-label">{{ __('sale.status_void') }}</div>
                <div class="stat-value text-danger">{{ $warrantySummary->voided ?? 0 }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="content-card">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    @endphp
                    <tr>
                        <td><strong class="text-teal"><i class="bi bi-shield-check"></i> {{ $warranty->warranty_code }}</strong></td>
                        <td><span class="badge bg-light text-dark border">{{ $warranty->shop->name }}</span></td>
                        <td><strong>{{ $warranty->sale->product->name ?? '-' }}</strong></td>
                        ...
                        <td>
                            <span class="status-pill status-pill-{{ $pillClass }}">
                                <span class="status-indicator"></span>
                                {{ __('sale.status_' . $effectiveStatus) }}
                            </span>
                        </td>
                        <td>
                            <span class="text-muted small text-truncate d-inline-block" style="max-width: 200px;" title="{{ $warranty->terms ?: $warranty->claim_note }}">
                                {{ $warranty->terms ?: ($warranty->claim_note ?: '-') }}
                            </span>
                        </td>
                        <td class="text-center">
                            <button
                                ...
                                data-warranty='@json($warrantyData, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_TAG)'
                            >
                                <i class="bi bi-eye"></i>
                            </button>
                        </td>
        ...
        <div class="p-5 text-center text-muted">
            ...
            <p class="mb-0">{{ __('report.no_warranties_found') }}</p>
        </div>
        @endif
    </div>
</div>
